...
def _fetch_images_from_child(child):
    ...
    for image in child.pictures_ids:
        ...
        filename = f"{child.preferred_name}_" f"{image.date}_{child.local_id}.{ext}"
...
class MyAccountController(PaymentFormController):
    ...
    def contact_us(self, **kwargs):
        ...
        return request.render(
            ...
            {"partner": partner, "form": claim_form},
        )
    ...
    def my_donations_update(self, recurring_contract=None, new_amount=None, **kw):
        ...
        try:
            ...
            max_amount = (
                write_and_pray_max if contract.type in ["SWP"] else sponsorship_max
            )
        ...
        if "confirmed" not in kw:
            upgrade_url = (
                f"/my/donations/upgrade?confirmed&recurring_contract={contract.id}"
            )
            if new_amount:
            ...
            return request.render(
                "website_compassion.my_donations_update_confirmation", context
            )
        ...
        def create_line(fund_name, amount):
            ...
            product_template = product_template.search(
                [("default_code", "=", fund_name)], limit=1
            )
            return (
                0,
                0,
    ...
    @route(
        "/my/donations/submit_have_parent_consent",
        type="http",
        auth="user",
        website=True,
    )
    ...
    def my_donations(self, invoice_page="1", form_id=None, invoice_per_page=30, **kw):
        ...
        invoice_per_page = (
            int(invoice_per_page)
            if isinstance(invoice_per_page, str)
            else invoice_per_page
        )
        invoice_page = (
            invoice_page
            if invoice_page >= 1
            and (invoice_page - 1) * invoice_per_page < all_invoice_count
            else 1
        )
        ...
        invoices = sudo_invoice_env.search(
            invoice_search_criteria,
        ...
        paid_sponsorships = (
            partner.contracts_fully_managed + partner.contracts_paid
        ...
        currency = (
            paid_sponsorships.mapped("invoice_line_ids.currency_id.name") or [False]
        )[0] or "CHF"
    ...
    def my_information(self, form_id=None, **kw):
        ...
        def get_form(form_model_key):
            form = self.get_form(
                "res.partner", partner.id, form_model_key=form_model_key, **kw
            )
    ...
    def save_ambassador_picture(self, **post):
        ...
        if picture_post:
            image_value = compress_big_images(
                b64encode(picture_post.stream.read()), None, None
            )
            if not image_value:
    ...
    def download_file(self, source, **kw):
        ...
        if source == "picture":
            ...
            wizard = (
                ...
                .with_context(active_ids=active_sponsorship.mapped("id"))
                .sudo()
                .create(
                    {
                        "pdf": True,
            ...
            pdf_data = (
                ...
                .sudo()
                .render_qweb_pdf(label_data["doc_ids"], data=label_data)[0]
            )
            ...
            return Response(
                b64decode(pdf_download), content_type="application/pdf", headers=headers
            )
