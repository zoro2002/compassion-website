...
class ContactUsForm(models.AbstractModel):
    ...
    def _form_fieldsets(self):
        fields = [
            {"id": "subject", "fields": ["subject", "partner_id"]},
            {"id": "question", "fields": ["name"]},
        ]
    ...
    def form_before_create_or_update(self, values, extra_values):
        ...
        category_ids = (
            self.env["crm.claim.category"].sudo().search([("keywords", "!=", False)])
        )
        category_id = False
        ...
        values.update(
            {
                ...
                "user_id": False,
                "language": self.env["langdetect"]
                .detect_language(values.get("name"))
                .lang_id.code,
