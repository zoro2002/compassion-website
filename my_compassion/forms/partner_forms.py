...
class PartnerCoordinatesForm(models.AbstractModel):
    ...
    def _form_validate_alpha_field(self, field, value, accept=""):
        if value and not re.match(rf"^[\w\s'-{accept}]+$", value, re.UNICODE):
...
class PartnerDeliveryForm(models.AbstractModel):
    ...
    _form_model_fields = ["lang", "spoken_lang_ids", "no_physical_letter"]
    _form_fields_order = [
        "lang",
        "spoken_lang_ids",
        "no_physical_letter",
    ]
    _form_required_fields = ["lang"]
