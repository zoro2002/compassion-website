...
class PaymentOptionsForm(models.AbstractModel):
    ...
    def _form_load_payment_mode(self, fname, field, value, **req_values):
        payment = self.main_object.with_context(lang="en_US").payment_mode_id.name
        return next(
            (
                payment
                for modes in self._fields["payment_mode"].selection
                if payment in modes
            ),
            None,
        )
    ...
    def _if_needed(self, dic):
        ...
        if (
            "payment_mode_id" in dic
            and dic["payment_mode_id"] == self.main_object["payment_mode_id"].id
        ):
    ...
    def form_extract_values(self, **request_values):
        values = super(PaymentOptionsForm, self).form_extract_values(**request_values)
            ...
            if values[key]:
                ...
                if values[key] == "Permanent Order":
                    if not self.bvr_reference:
                        raise ValueError("Permanent Order needs a BVR reference.")
                    group_vals.update(
                        {
...
class PaymentOptionsMultipleForm(models.AbstractModel):
    ...
    def form_extract_values(self, **request_values):
        def filter_fun(c):
            return c.state not in ["cancelled", "terminated"] and partner == c.mapped(
                "partner_id"
            )
